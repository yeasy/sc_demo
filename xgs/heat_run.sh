...
if [ -n "`heat stack-list|grep \"${STACK}\"`" ]; then 
    heat stack-update $STACK -f ./xgs_template.yaml --parameters="user_tenant_id=${TENANT_ID};user_image_id_1=${IMG_USER_ID1};user_image_id_2=${IMG_USER_ID2};xgs_image_id=${IMG_XGS_ID};routed_image_id=${IMG_ROUTED_ID};user_flavor=ex.tiny"
else
    heat stack-create $STACK -f ./xgs_template.yaml --parameters="user_tenant_id=${TENANT_ID};user_image_id_1=${IMG_USER_ID1};user_image_id_2=${IMG_USER_ID2};xgs_image_id=${IMG_XGS_ID};routed_image_id=${IMG_ROUTED_ID};user_flavor=ex.tiny"
fi
